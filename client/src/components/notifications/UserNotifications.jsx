...
const UserNotifications = ({ visible, onClose }) => {
    ...
    const viewNotificationDetail = async (notification) => {
        ...
        // Refresh membership data if cancellation was rejected (membership should be restored)
        if (notification.Type === 'cancellation_rejected') {
            console.log('🔍 Received cancellation_rejected notification, refreshing membership...');
            try {
                ...
                if (window.refreshMembershipData) {
                    ...
                    await window.refreshMembershipData();
                }

                notification.success({
                    message: 'Gói dịch vụ đã được khôi phục',
                    description: 'Gói dịch vụ của bạn đã được khôi phục và tiếp tục hoạt động bình thường. Hãy kiểm tra trang Gói dịch vụ.',
                    duration: 8
                });
