...
async function fixAchievementsDatabase() {
    ...
    try {
        ...
        // Check if UserAchievements table exists before deleting
        const userAchievementsCheck = await pool.request().query(`
            SELECT COUNT(*) as count 
            FROM INFORMATION_SCHEMA.TABLES 
            WHERE TABLE_NAME = 'UserAchievements'
        `);

        if (userAchievementsCheck.recordset[0].count > 0) {
            await pool.request().query('DELETE FROM UserAchievements');
            console.log('✅ UserAchievements cleared');
        }
        ...
        if (error.code === 'ELOGIN') {
            ...
            console.log('3. Database "SMOKEKING" exists');
