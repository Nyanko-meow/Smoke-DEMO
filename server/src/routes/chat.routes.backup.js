...
// Test route for debugging member authorization
router.get('/test-member-auth', authenticateToken, authorizeRole(['member']), (req, res) => {
    console.log('🧪 Test member auth route - req.user:', req.user);
    res.json({
        success: true,
...
// Lấy conversation của member với coach (simplified version without role check)
router.get('/member/conversation-simple', authenticateToken, async (req, res) => {
    try {
        const userId = req.user.UserID;
        console.log('🔍 Member conversation simple - UserID:', userId, 'Role:', req.user.Role);
